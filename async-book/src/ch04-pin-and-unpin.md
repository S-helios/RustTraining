...
This isn't an academic concern. An async state machine **may** become
self-referential when a value kept across `.await` borrows another field stored
inside that same future. A borrowed function argument can instead point outside
the future and is not, by itself, a self-reference. The following is the
...
Most types in Rust are `Unpin`, so pinning does not restrict moving them.
Compiler-generated async state machines are commonly `!Unpin`, but they are not
the only examples: a custom type can opt out of `Unpin`, usually with
`PhantomPinned`, when its safety invariants depend on a stable address.

```rust
// These are all Unpin — pinning them does nothing special:
// i32, String, Vec<T>, HashMap<K,V>, Box<T>, &T, &mut T

// Common !Unpin examples that must be pinned before address-sensitive use:
// state machines generated by async fn / async {}, and custom PhantomPinned types

// Practical implication:
// A hand-written future whose fields are all Unpin normally gets Unpin
// automatically. Opting out is deliberate; manually implementing Unpin should
// follow from the type's pinning invariants, not from convenience alone.
```

> **Deep understanding — Pin fixes the value, not the pointer wrapper**
>
> Moving `Pin<Box<T>>` or `Pin<&mut T>` is fine: the pointer or reference moves,
> while the underlying `T` stays at the same address. Once a `!Unpin` value is
> pinned, safe APIs will not move that value before destruction completes.
