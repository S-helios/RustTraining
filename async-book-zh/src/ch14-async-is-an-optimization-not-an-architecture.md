...
常见反驳是“线程昂贵，所以必须异步”。在大多数团队的负载规模下，这通常并不成立。

- **栈内存：** Linux 默认每个线程预留约 8 MB 虚拟地址空间，但只按实际触碰的页面提交物理内存；多数空闲线程可能只用 20–80 KB。
- **上下文切换：** 现代硬件约 1–5 微秒。50 个并发请求时几乎可以忽略，每秒十万次时才明显。
- **创建成本：** Linux 上每线程约 10–30 微秒；线程池（Rayon、`std::thread::scope`）可以摊薄成本。

异步开始值得其复杂度的粗略范围，是 **1,000–10,000 个大部分时间空闲的并发连接**：这正是 epoll/io_uring 擅长的区间，每连接线程栈开始成为真实成本。低于它，线程池往往更简单、易调试且足够快；高于它，异步优势明显。阈值不是定律，应以测量为准。

## 困难案例：逻辑中间也需要 I/O

纯函数显然不需要异步。真正值得讨论的是业务逻辑中间*看起来*必须 I/O 的情况：验证库存、查询汇率定价、在订单流程中查找客户。
...
这段异步代码本身很合理，没有滥用 `Arc<Mutex>`，只是顺序等待。但 `validate_items`、`validate_quantities`、`calculate_pricing`、`Receipt::new` 都是纯逻辑，仅因两个步骤需要 I/O，整个函数、测试和上层调用者都被染成异步。
...
这通常说明：**这些逻辑本来就不是异步的。** 真正需要的是由异步处理器直接调用的同步模块，而不是更大的 `spawn_blocking`。
...
`spawn_blocking` 应留给真正繁重、会饿死执行器的 CPU 工作，例如解析巨型载荷、图像处理、压缩。只需微秒的普通业务逻辑直接同步调用即可。
...
- **长生命周期有状态连接。** WebSocket、gRPC 双向流和协议状态机的转换天然与 I/O 事件绑定。第 17 章聊天服务器中的并发连接、房间广播和优雅关闭正是此类工作。

**判断方法：** 若移除 `async` 后必须用线程、通道或手工轮询替代，异步确实在发挥作用；若只是删掉关键字即可，它从来就不需要异步。
...
- 定义小型测试辅助函数构造测试夹具，例如 `raw_telemetry()`、`sensor()`、`reading()`、`device_meta()`。
