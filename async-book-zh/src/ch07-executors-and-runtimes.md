...
就绪模型（epoll / mio / Tokio）：
  ...
  3. 应用：非阻塞 read(fd, buf)              → 返回数据；就绪状态变化时返回 EAGAIN
...
| **复制行为** | 通常在内核与用户空间之间复制 | 已登记缓冲区可减少重复登记或固定开销；是否零拷贝取决于具体操作 |
| **成熟度** | 受支持运行时上的成熟默认方案 | 已用于部分生产场景，但生态与运维取舍仍取决于工作负载 |

> **何时使用 io_uring**：只有性能分析表明系统调用提交/完成开销，或面向完成通知的 API 确实影响当前负载时，才值得考虑。应在目标内核、文件系统、设备和运行时上实测；连接数高并不自动意味着更快。对大多数应用，标准 Tokio 仍是更简单的默认选择。
