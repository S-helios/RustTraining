...
这不是纯理论问题。当一个跨 `.await` 保存的值借用了同一个 Future 内部保存的另一个字段时，异步状态机**可能**变成自引用结构。函数参数中的借用也可能只是指向 Future 外部，仅凭“跨 `.await` 持有引用”并不能断定它是自引用。下面是用于教学的典型例子：
...
Rust 中的大多数类型都是 `Unpin`，因此固定不会限制它们被移动。编译器生成的异步状态机通常是 `!Unpin`，但并非只有 `!Unpin` 类型如此：当自定义类型的安全不变量依赖稳定地址时，也可以通过 `PhantomPinned` 等方式主动退出 `Unpin`。

```rust
// These are all Unpin — pinning them does nothing special:
// i32, String, Vec<T>, HashMap<K,V>, Box<T>, &T, &mut T

// 常见的 !Unpin 类型在涉及地址敏感操作前必须固定：
// async fn / async {} 生成的状态机，以及自定义 PhantomPinned 类型

// 手写 Future 若所有字段都是 Unpin，通常会自动获得 Unpin。
// 是否手动实现 Unpin 应由该类型的固定不变量决定，而不是仅为了使用方便。
```
...
**片段 B**：✅ **可以编译。** `tokio::pin!` 把 Future 固定在栈上，并把 `fut` 重新绑定为 `Pin<&mut ...>`。`let moved = fut` 移动的是 **`Pin` 包装器**（一个指针），不是底层 Future；Future 仍固定在栈上的原位置。这与 `Box::pin` 后移动 `Box` 不会搬动堆分配类似。但 `fut` 会被这次移动消耗，之后只能使用 `moved`，不能再使用原来的 `fut`：
...
> - `Unpin` 是一个自动 trait：实现 `Unpin` 的类型即使已固定也可以移动（大多数类型是 `Unpin`，异步块不是）
